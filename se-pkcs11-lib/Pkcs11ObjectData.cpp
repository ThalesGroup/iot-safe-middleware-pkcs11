...
bool DataObject::isEqual( StorageObject * that) const
{
    ...
    const DataObject * thatData = static_cast< const DataObject* >( that );

	return ( Util::compareU1Arrays(m_pApplicationName.get(), thatData->m_pApplicationName->GetBuffer(), thatData->m_pApplicationName->GetLength())
		&& Util::compareU1Arrays(m_pID.get(), thatData->m_pID->GetBuffer(), thatData->m_pID->GetLength())
		&& Util::compareU1Arrays(m_pValue.get(), thatData->m_pValue->GetBuffer(), thatData->m_pValue->GetLength()) );
}
